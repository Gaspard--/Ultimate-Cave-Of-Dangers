...
void my_opengl::setUniform(Vect<float, 2> const data, char const *target, Program program)
{
  glUniform2f(glGetUniformLocation(program, target), data[0], data[1]);
}

void my_opengl::setUniform(Vect<float, 3> const data, char const *target, Program program)
{
  glUniform3f(glGetUniformLocation(program, target), data[0], data[1], data[2]);
}

void my_opengl::setUniform(Vect<float, 4> const data, char const *target, Program program)
{
...
Texture my_opengl::loadTexture(std::string const &name)
{
  ...
  char readBuf[4];
  Vect<unsigned int, 2u> dim{0u, 0u};
  try {
    file.seekg(10);
